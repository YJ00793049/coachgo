...
export function ClubhouseNav({
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      setDirection(1);
      setActiveIdx((i: number) => (i + 1) % CLUBHOUSE_DATA.length);
    }, 5000);
  ...
  const prev = () => goTo((activeIdx - 1 + CLUBHOUSE_DATA.length) % CLUBHOUSE_DATA.length);
  const next = () => goTo((activeIdx + 1) % CLUBHOUSE_DATA.length);
    ...
    >
      <MeshCard className="relative overflow-hidden">
        <div className="flex flex-col md:flex-row min-h-[420px] md:min-h-[460px]">
          ...
          {/* Left: tabs + text */}
          <div className="flex flex-col justify-center gap-6 p-8 md:p-12 md:w-[46%] shrink-0">
            {/* Discipline tabs */}
            <div className="flex flex-wrap gap-2">
              {CLUBHOUSE_DATA.map((z, i) => (
                <button
                  ...
                  type="button"
                  onClick={() => goTo(i)}
                  className="text-[11px] tracking-widest uppercase px-3 py-1.5 rounded-full transition-all duration-200"
                  style={{
                    background: i === activeIdx ? 'var(--ink)' : 'transparent',
                    color: i === activeIdx ? 'var(--paper)' : 'var(--ink-faint)',
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <h3 className="display-md" style={{ whiteSpace: 'pre-line' }}>
                  {zone.tagline}
                </h3>
                <p className="text-sm leading-relaxed" style={{ color: 'var(--ink-soft)', maxWidth: 360 }}>
                  {zone.desc}
                </p>
                <div className="flex flex-col gap-2 mt-1">
                  ...
                  {counts && counts[zone.slug] ? (
                    <span className="text-xs" style={{ color: 'var(--ink-faint)' }}>
                      {counts[zone.slug]} coaches available
                    </span>
                  ) : null}
                </div>
              </motion.div>
          ...
          <div className="relative flex-1 min-h-[260px] md:min-h-0 overflow-hidden">
            ...
            <button
              ...
              className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center opacity-80 hover:opacity-100 transition-opacity"
              style={{ background: 'rgba(251,250,246,0.92)', border: '1px solid var(--line)', color: 'var(--ink)' }}
            >
              <ChevronLeft size={16} />
            </button>
            <button
              ...
              className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center opacity-80 hover:opacity-100 transition-opacity"
              style={{ background: 'rgba(251,250,246,0.92)', border: '1px solid var(--line)', color: 'var(--ink)' }}
            >
              ...
              {CLUBHOUSE_DATA.map((z, i) => (
                <button
                  ...
                  style={{
                    ...
                    height: 6,
                    background: i === activeIdx ? 'rgba(251,250,246,0.95)' : 'rgba(251,250,246,0.50)',
                  }}
                />
              ))}
            </div>
