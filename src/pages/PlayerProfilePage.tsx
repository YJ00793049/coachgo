    ...
    <PageTransition>
      <div className="min-h-screen pt-20" style={{ background: '#0A0F1E' }}>
        ...
        <div className="relative overflow-hidden" style={{ background: 'rgba(79,142,247,0.05)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(circle at 50% 50%, rgba(79,142,247,0.07) 0%, transparent 70%)' }} />
          <div className="max-w-4xl mx-auto px-4 sm:px-6 py-16 relative z-10">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] mb-4" style={{ color: 'rgba(255,255,255,0.3)' }}>My Profile</p>
              <div className="flex items-center gap-6">
                <div className="w-20 h-20 rounded-3xl flex items-center justify-center shrink-0"
                  ...
                  <span className="text-3xl font-bold text-white">{(displayName || user.email || 'P').charAt(0).toUpperCase()}</span>
                </div>
                <div>
                  <h1 className="font-display text-4xl md:text-5xl text-white">{displayName || 'Player'}</h1>
                  <p style={{ color: 'rgba(255,255,255,0.4)' }} className="mt-1">{user.email}</p>
                </div>
              </div>
            </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              ...
              <h2 className="font-display text-2xl text-white mb-6 flex items-center gap-3">
                <User size={20} style={{ color: '#4F8EF7' }} /> Display Name
              </h2>
              {editing ? (
                <div className="space-y-4">
                  <input
                    ...
                    className="w-full rounded-2xl px-4 py-3 text-white focus:outline-none"
                    style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(79,142,247,0.4)' }}
                  ...
                  <div className="flex gap-3">
                    ...
                    <button onClick={() => { setEditing(false); setDisplayName(user.displayName || ''); }}
                      className="px-4 py-3 rounded-2xl text-sm font-bold transition-all hover:bg-white/5"
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
              className="glass-card rounded-3xl border border-white/10 p-8">
              <h2 className="font-display text-2xl text-white mb-6 flex items-center gap-3">
                <CheckCircle2 size={20} style={{ color: '#4F8EF7' }} /> Account
              </h2>
              <div className="space-y-4">
                {[
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              className="glass-card rounded-3xl border border-white/10 p-8 md:col-span-2">
              <h2 className="font-display text-2xl text-white mb-6">Quick Actions</h2>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[
                  { label: 'Browse Coaches', to: '/coaches', color: '#4F8EF7' },
                  { label: 'My Dashboard', to: '/dashboard', color: '#22c55e' },
                  { label: 'Saved Coaches', to: '/dashboard', color: '#F59E0B', icon: <Heart size={16} /> },
                ].map(link => (
                  <Link key={link.label} to={link.to}
                    className="flex items-center justify-center gap-2 py-3.5 rounded-2xl text-sm font-bold transition-all hover:opacity-80 border border-white/8"
